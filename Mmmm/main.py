...
def no_projects(message):
    bot.send_message(message.chat.id, 'У тебя пока нет рыбешки!\nМожешь добавить её с помошью команды /new_fish')
...
attributes_of_projects = {'Имя рыбы' : ["Введите новое имя рыбы", "project_name"],
                          "Описание" : ["Введите новое описание рыбы", "description"],
                          "Ссылка" : ["Введите новую ссылку на тна рыбы", "url"],
...
def start_command(message):
    bot.send_message(message.chat.id, """Дорово! Я опытный рыбакп
Я хочу узнать больше о твоих рыбешека также сохроню информацию о них!) 
""")
    info(message)
...
/new_fish - используй для добавления нового рыбы
/delete - удаляет твою рыбы по твоему запросу
/weight - добовляет вес твоей рыбы
/projects - показывает твоих рыб
/update_projects - изминает статус твоей рыбы

""")
    

@bot.message_handler(commands=['new_fish'])
def addtask_command(message):
    bot.send_message(message.chat.id, "Введите название рыбы:")
...
def name_project(message):
    ...
    data = [user_id, name]
    bot.send_message(message.chat.id, "Введите ссылку на фото вашей рыбы")
    bot.register_next_step_handler(message, link_project, data=data)

def link_project(message, data):
    data.append(message.text)
    statuses = [x[0] for x in manager.get_statuses()] 
    bot.send_message(message.chat.id, "Отпустил или забрал рыбешку", reply_markup=gen_markup(statuses))
...
def callback_project(message, data, statuses):
    ...
    bot.send_message(message.chat.id, "Рыбешка сохранина")
...
def skill_handler(message):
    ...
    if projects:
        ...
        bot.send_message(message.chat.id, 'Выбери рыбу для которого нужно выбрать вес', reply_markup=gen_markup(projects))
...
def skill_project(message, projects):
    ...
    if project_name not in projects:
        bot.send_message(message.chat.id, 'У тебя нет такой рыбы, попробуй еще раз!) Выбери рыбку для которого нужно выбрать вес', reply_markup=gen_markup(projects))
        bot.register_next_step_handler(message, skill_project, projects=projects)
    else:
        skills = [x[1] for x in manager.get_skills()]
        bot.send_message(message.chat.id, 'Выбери вес', reply_markup=gen_markup(skills))
...
def set_skill(message, project_name, skills):
    ...
    if skill not in skills:
        bot.send_message(message.chat.id, 'Видимо, ты выбрал вес. не из спика, попробуй еще раз!) Выбери вес', reply_markup=gen_markup(skills))
        bot.register_next_step_handler(message, set_skill, project_name=project_name, skills=skills)
        return
    manager.insert_skill(user_id, project_name, skill )
    bot.send_message(message.chat.id, f'Вес {skill} добавлен к рыбешке {project_name}')
...
def delete_project(message, projects):
    ...
    if project not in projects:
        bot.send_message(message.chat.id, 'У тебя нет такой рыбы, попробуй выбрать еще раз!', reply_markup=gen_markup(projects))
        bot.register_next_step_handler(message, delete_project, projects=projects)
        return
    project_id = manager.get_project_id(project, user_id)
    manager.delete_project(user_id, project_id)
    bot.send_message(message.chat.id, f'рыбка {project} удален!')
...
def update_project(message):
    ...
    if projects:
        projects = [x[2] for x in projects]
        bot.send_message(message.chat.id, "Выбери рыбу, который хочешь изменить", reply_markup=gen_markup(projects))
...
def update_project_step_2(message, projects):
    ...
    if message.text == cancel_button:
        ...
        return
    if project_name not in projects:
        bot.send_message(message.chat.id, "Что-то пошло не так!) Выбери рыбу, которую хочешь изменить еще раз:", reply_markup=gen_markup(projects))
        bot.register_next_step_handler(message, update_project_step_2, projects=projects )
        return
    bot.send_message(message.chat.id, "Выбери, что требуется изменить в рыбе", reply_markup=gen_markup(attributes_of_projects.keys()))
...
def text_handler(message):
    ...
    if message.text == 'Текст': #текст обычной не инлайн кнопки
        bot.send_message(message.chat.id, 'Зараз', reply_markup=None)
